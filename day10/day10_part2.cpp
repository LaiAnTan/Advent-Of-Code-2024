...
)
{
	...
	for (std::pair<int, int> d : directions)
	{
		...
		{
			followTrailheadAux(topological_map, n, unique_trails);
		}
	}
}

int	followTrailhead(std::vector<std::vector<int>> topological_map, std::pair<int, int> start_coords)
{
	int unique_trails = 0;
	followTrailheadAux(topological_map, start_coords, unique_trails);
...
int main(int argc, char **argv)
{
	...
	{
		int unique_trails = followTrailhead(topological_map, th);
