...
bool dijkstra(Coordinate start, Coordinate end, std::vector<std::vector<char>> board)
{
	// up, down, left, right
	...
	return ((dist[end]) != INT_MAX ? true : false);
}
...
int main(int argc, char **argv)
{
	...
	simulateObstaclePlacement(obstacles, nth_obstacle, board);
	...
	int total_obstacles = obstacles.size();

	for (int i = nth_obstacle; i < total_obstacles; i++)
	{
		board[obstacles[i].second][obstacles[i].first] = '#';
		if (!dijkstra(start, end, board))
		{
			std::cout << "Obstacle " << i << " at " << obstacles[i] << " caused there to be no path to the end" << std::endl;
			break;
		}
	}
}